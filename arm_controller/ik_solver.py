...
class InverseKinematicsSolver:
    def __init__(self):
        ...
        self.min_angle = np.radians(-45) 
        self.max_angle = np.radians(90)  

        # 初期角度
        self.theta1 = np.radians(30.0)
        self.theta2 = np.radians(30.0)
        self.theta3 = np.radians(30.0)
    ...
    def newton_raphson_ik(self, x_target, z_target, alpha_target, max_iterations=100, tolerance=0.01,tolerance_alpha=0.01):
        ...
        for i in range(max_iterations):
            ...
            self.theta2 = np.clip(self.theta2 + delta_theta[1], self.min_angle, self.max_angle)
            self.theta3 = np.clip(self.theta3 + delta_theta[2], self.min_angle, self.max_angle)
